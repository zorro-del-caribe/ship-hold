...
const belongsTo = (targetBuilder, relation, sh) => {
    ...
    const selectValue = {
        value: shipHoldQuerybuilder.toJson(`"${as}".*`),
        as
    };
...
const hasOne = (targetBuilder, relation, sh) => {
    ...
    return targetBuilder
        .select({
        value: sh.select({ value: shipHoldQuerybuilder.toJson(`"${as}".*`), as: as })
            .from(as)
            .where(selectLeftOperand, selectRightOperand)
            .noop()
    })
...
const coalesceAggregation = (arg) => shipHoldQuerybuilder.coalesce([shipHoldQuerybuilder.jsonAgg(arg), `'[]'::json`]); // we return empty array instead of null
