...
const oneToMany = has(shipHoldQuerybuilder.jsonAgg);
const manyToOne = (targetBuilder, relationBuilder) => {
    ...
    return targetBuilder
        .select({
        value: relSelect(shipHoldQuerybuilder.toJson(`"${relTable}".*`, alias)).where(withLeftOperand, selectRightOperand)
            .noop()
    })
        .with(relTable, 
    // @ts-ignore
...
const oneBelongsToOne = (targetBuilder, relationBuilder) => {
    ...
    return targetBuilder
        .select({
        value: relSelect(shipHoldQuerybuilder.toJson(`"${relationTable}".*`, alias))
