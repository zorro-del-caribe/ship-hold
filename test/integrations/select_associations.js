    ...
    `;
		...
		t.test('one to one: load al users with their phone number (builder notation)', async t => {
			...
			const builder = Users
				.select()
				.orderBy('id')
				.include(Phones.select('number', 'user_id'));

			t.deepEqual(builder.build().text, `SELECT "users_association_select".*, "phone"."number" AS "phone.number", "phone"."user_id" AS "phone.user_id" FROM (SELECT * FROM "users_association_select" ORDER BY "id") AS "users_association_select" LEFT JOIN (SELECT "number", "user_id" FROM "phones_association_select") AS "phone" ON "users_association_select"."id" = "phone"."user_id" ORDER BY id`);
		...
		/*
		t.test('one to Many: load al users with their products (string notation)', t => {
		...
		t.test('one to many: specify fields (builder notation)', t => {
			const {Users, Products} = createModels();
			Users
				.select('id', 'name')
				.orderBy('id')
		...
		t.test('one to many: filter on target model', t => {
			const {Users, Products} = createModels();
			Users
				.select('id', 'name')
				.where('name', 'Laurent')
		...
		t.test('one to many: order and limit on target model', t => {
			const {Users, Products} = createModels();
			Users
				.select('id', 'name', 'age')
				.where('age', '<', 50)
		...
		t.test('one to many:filter on included model', t => {
			const {Users, Products} = createModels();
			const u = Users
				.select('id', 'name')
				.where('name', 'Laurent')
		...
		t.test('many to one: specify fields (string notation)', t => {
			const {Products, Users} = createModels();
		...
		t.test('many to one: filter on target model', t => {
			const {Products, Users} = createModels();
			Products
				.select('title', 'id')
		...
		t.test('many to one: order and limit on target model', t => {
			const {Products, Users} = createModels();
			Products
		...
		t.test('include multiple models', t => {
		...
		t.test('many to many filter on include', t => {
			const {Users, Accounts} = createModels();
			Users
				.select('id', 'name')
				.orderBy('id')
