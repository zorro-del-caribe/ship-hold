...
const jsonPointer = path => ({
	...
	set: (target, newTree) => {
		...
		current[leaf] = Array.isArray(newTree) ? (current[leaf] || []).concat(newTree) : Object.assign(current[leaf] || {}, newTree);

		return target;
	}
});

exports.default = path => path === undefined ? rootPointer() : jsonPointer(path);