...
function includeFactory (service) {
  return function include () {
    ...
    for (const m of associations) {
      ...
      const targetModel = sh.model(m.model);
      const targetKey = targetModel.keyWith(service.table);
      newQueryBuilder
        .leftJoin({value: targetModel.table, as: m.association})
        .on([table, sourceKey].join('.'), `"${m.association}"."${targetKey}"`)
        .noop();
    }
    ...
    Object.assign(newQueryBuilder, sources);
